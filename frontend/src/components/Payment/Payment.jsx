    ...
    try {
      if (dropInInstance) {
        dropInInstance.requestPaymentMethod(async (requestPaymentMethodErr, payload) => {
          ...
          try {
            const response = await fetch(`${server}/api/v2/payment/checkout`, {
              method: 'POST',
              headers: {
...
loading,
  ...
  return (
    ...
    <div className={`w-full 800px:w-[95%] bg-[#fff] rounded-md p-5 pb-8 ${loading ? 'blur-sm' : ''}`}>
      ...
      <button  className={`${styles.button} !bg-[#f63b60] text-white h-[45px] rounded-[5px] cursor-pointer text-[18px] font-[600]`}  onClick={paymentHandler}>Confirm</button>
          </div>
        ) : null}
